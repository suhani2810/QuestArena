...
class _GuildResultScreenState extends ConsumerState<GuildResultScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return matchAsync.when(
      data: (match) {
        ...
        final int myGuildScore =
            isGuildA ? match.guildAScore : match.guildBScore;
        final int oppGuildScore =
            isGuildA ? match.guildBScore : match.guildAScore;
        final bool isWin = myGuildScore > oppGuildScore;
        final String myGuildName =
            isGuildA ? match.guildAName : match.guildBName;
        final String oppGuildName =
            isGuildA ? match.guildBName : match.guildAName;
        final String myGuildIcon =
            isGuildA ? match.guildAIcon : match.guildBIcon;
        ...
        return Scaffold(
          ...
          body: NeonSwirlBackground(
            colors: isWin
                ? const [AppColors.teal, AppColors.neonCyan]
                : const [AppColors.red, AppColors.bgBase],
            child: Stack(
              children: [
                ConfettiWidget(
                      ...
                      children: [
                        ...
                        Text(
                          ...
                          style: AppTextStyles.label.copyWith(
                              letterSpacing: 4, color: AppColors.textSecondary),
                        ),

                        const SizedBox(height: 40),

                        // GUILD VS GUILD SCORE
                        _buildGuildScoreBoard(myGuildName, myGuildIcon,
                            myGuildScore, oppGuildName, oppGuildScore, isWin),

                        const SizedBox(height: 32),

                        // PLAYER CONTRIBUTION
                        _buildContributionCard(
                            user, match.playerScores[user.uid] ?? 0, isWin),
  ...
  Widget _buildGuildScoreBoard(String myName, String myIcon, int myScore,
      String oppName, int oppScore, bool isWin) {
    return Container(
      padding: const EdgeInsets.all(24),
      decoration: BoxDecoration(
        ...
        border: Border.all(
            color: isWin ? AppColors.gold : AppColors.surface, width: 2),
      ),
      child: Column(
        children: [
          Row(
            ...
            children: [
              _GuildScoreColumn(
                  ...
                  score: myScore,
                  iconId: myIcon,
                  color: isWin ? AppColors.gold : AppColors.textSecondary),
              Text('VS',
        ...
        children: [
          ...
          Row(
            ...
            children: [
              ...
              _StatMini(
                  label: 'GUILD XP', value: '+$xp', color: AppColors.purple),
              _StatMini(
                  label: 'COINS', value: '+$coins', color: AppColors.gold),
            ],
          ),
        ],
      ),
  ...
  Future<void> _captureAndShare(
      GuildBattleMatchModel match, UserModel user, bool isWin) async {
    try {
      final bool isGuildA = match.guildAId == user.guildId;
      ...
      final imagePath =
          await File('${directory.path}/guild_victory.png').create();
      await imagePath.writeAsBytes(image);

      const shareMessage =
          "My guild just dominated in QuestArena! 🛡️🔥\nJoin us and climb the ranks!\n\n🎮 Play now:\nhttps://quest-arena-self.vercel.app/";
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('Failed to share victory card: $e'),
  ...
  Widget _buildActions(
      GuildBattleMatchModel match, UserModel user, bool isWin) {
    return Column(
      children: [
        if (isWin) ...[
          SizedBox(
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        Text('$score',
            style: AppTextStyles.display.copyWith(fontSize: 32, color: color)),
      ],
    );
  }
