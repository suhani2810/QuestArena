...
class _ResultScreenState extends ConsumerState<ResultScreen> {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _rewardsClaimed = true;
          if (result != null) {
            _leveledUp = LevelSystem.getCurrentLevel(
                    currentUser.xp + result.xpRewards.total) >
                oldLevel;
          }
        });
      }
  ...
  void _showVictoryCardPopUp(UserModel user, String opponentName, int myScore,
      int opponentScore, int xp, int coins, bool isMvp) {
    showDialog(
      context: context,
  ...
  void _onRematchPressed() async {
    ...
    await ref
        ...
        .requestRematch(widget.room.roomId, user.uid);
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (!widget.isPractice) {
      ref.listen<AsyncValue<GameRoomModel?>>(
          gameRoomProvider(widget.room.roomId), (prev, next) {
        ...
        if (room.nextMatchId != null && mounted) {
          Navigator.of(context).pushReplacement(
            MaterialPageRoute(
                builder: (_) => GameScreen(roomId: room.nextMatchId!)),
          );
        }
    ...
    final otherRequested = !widget.isPractice &&
        roomState.rematchRequests.any((id) => id != currentUser.uid);
    final waitingForOpponent = !widget.isPractice &&
        _rematchRequested &&
        roomState.rematchRequests.length < 2;
                ...
                children: [
                  ...
                  if (widget.isPractice || isDraw || !isWinner)
                    Text(
                      isWinner
                          ? "Awesome battle!"
                          : (isDraw ? "Well played!" : "Keep practicing!"),
                  ...
                  if (!widget.isPractice && _matchResult != null) ...[
                    ...
                    Text('No rewards earned in Practice Mode',
                    ...
                    if (_rematchTimer > 0 && roomState.nextMatchId == null) ...[
                      if (waitingForOpponent)
                        ...
                        ElevatedButton.icon(
                          ...
                          style: ElevatedButton.styleFrom(
                            backgroundColor: otherRequested
                                ? AppColors.teal
                                : AppColors.surface,
                      ...
                      const SizedBox(height: 12),
                      Text('Offer expires in $_rematchTimer s',
  ...
  Widget _buildScoreCard(int myScore, int opponentScore) {
    return Container(
      ...
      child: Column(
        children: [
          _ResultRow(
              label: 'YOUR SCORE', value: '$myScore', color: AppColors.gold),
          const Divider(color: AppColors.surface, height: 24),
          _ResultRow(
              label: widget.isPractice ? 'AI BOT' : 'OPPONENT',
              value: '$opponentScore',
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Text(pointsText,
                  style: AppTextStyles.label.copyWith(
                      color: pointsColor, fontWeight: FontWeight.bold)),
            ],
          ),
          const SizedBox(height: 16),
          Row(
            ...
            children: [
              RankBadge(
                  rank: rankUpdate.oldRank,
                  subRank: rankUpdate.oldSubRank,
                  size: 50),
...
class _VictoryCardModalState extends State<_VictoryCardModal> {
  ...
  Future<void> _captureAndShare() async {
    try {
      ...
      final imagePath =
          await File('${directory.path}/victory_card.png').create();
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        Text(value,
            style: AppTextStyles.display.copyWith(fontSize: 24, color: color)),
      ],
    );
  }
}
